...
ggplot(stage_predictions, aes(x = variety_label, y = residual)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  geom_boxplot(fill = "#4393c3", alpha = 0.3) +
  coord_flip() +
  facet_wrap(~stage) +
  labs(title = "Which varieties does the model explain well, and where does it struggle?",
       subtitle = "Positive = model underestimated actual yield; negative = model overestimated"
       , x = NULL, y = "Residual (actual \u2212 predicted yield, tons/acre)") +
  theme_minimal()
```
