...
```{r yield-vs-quality-summary}
#| echo: false
#| warning: false
#| message: false
#| fig-width: 7
#| fig-height: 5
#| fig-cap: "Figure 13: Yield has climbed while the climate signal tied to wine quality has eroded"
yield_trend <- statewide %>%
  filter(variety_label == "All", !is.na(production_tons), !is.na(acres_harvested), acres_harvested > 100) %>%
  transmute(yr, metric = "Yield (tons/acre)\nHigher = more productive", value = production_tons / acres_harvested)

diurnal_trend <- statewide %>%
  distinct(yr, diurnal_range_ripening) %>%
  transmute(yr, metric = "Ripening diurnal range (\u00b0C)\nHigher = better for Pinot Noir quality", value = diurnal_range_ripening)

summary_trends <- bind_rows(yield_trend, diurnal_trend)

ggplot(summary_trends, aes(x = yr, y = value, color = metric)) +
  geom_line(alpha = 0.4, linewidth = 0.5) +
  geom_smooth(method = "loess", se = FALSE, linewidth = 1.3) +
  scale_color_manual(values = c(
    "Yield (tons/acre)\nHigher = more productive" = "#8B1E3F",
    "Ripening diurnal range (\u00b0C)\nHigher = better for Pinot Noir quality" = "#4A7CC7"
  )) +
  facet_wrap(~metric, scales = "free_y", ncol = 1, strip.position = "left") +
  labs(title = "Two trends moving in opposite directions",
       subtitle = "Statewide yield (bottom) vs. ripening-season diurnal temperature range (top), 1981\u20132024",
       x = NULL, y = NULL) +
  theme_minimal() +
...
*rewrite suggestion* Read alongside the industry's own documented response, an interesting picture emerges.Over three decades, the consolidation around Pinot Noir and the decline in Chardonnay and Riesling shares occurred over the same period that Oregon's climate indicators shifted.

Read alongside the industry's own documented response, a three-decade consolidation around Pinot Noir, with Chardonnay and Riesling's statewide share declining over the same period Oregon's climate indicators were shifting, an interesting picture emerges. Pinot Noir shows a moderate positive yield relationship with heat accumulation in our data, comparable to Pinot Gris and stronger than several other varieties, so nothing in our yield findings suggests this consolidation has come at a yield cost. But the diurnal-range finding is the one piece of evidence in this entire report that has not yet shown up in a production number, a leading indicator for a wine-quality risk that a yield-only model, ours included, is structurally unable to see.
